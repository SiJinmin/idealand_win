...
IdealandThreadInfo *idealand_thread_by_id(INT32 id, int print)
{
  ...
  if (print)
    printf("cannot get thread by id %I32d\n", id);
  return NULL;
}
...
char *idealand_thread_log_path(INT32 id, int end)
{
  if (id < 0)
  {
    printf("thread id cannot be less than 0: %I32d\n", id);
    return NULL;
  }
  ...
  if (end)
  {
    path = idealand_string(2048, NULL, "%s%.3f_%I32d.log", IdealandLogsPath, idealand_time_yyyyMMddHHmmss(), id);
  }
  else
  {
    path = idealand_string(2048, NULL, "%s%I32d.log", IdealandLogsPath, id);
  }
  return path;
}
...
{
  if (id < 0)
    id = (INT32)GetCurrentThreadId();
  IdealandThreadInfo *p = idealand_thread_by_id(id, 0);
  if (p == NULL)
    p = idealand_thread_by_id();
  if (p == NULL)
    return;
  p->id = id;
  if (p->log_path == NULL)
  {
    char *path = idealand_thread_log_path(id);
    if (path == NULL)
      return;
    p->log_path = path;
...
void idealand_thread_end(INT32 id)
{
  ...
  if (p == NULL || p->pLog == NULL || p->log_path == NULL)
    return;
  ...
  fclose(p->pLog);
  char *path_end = idealand_thread_log_path(id, 1);
  if (path_end == NULL)
    return;
