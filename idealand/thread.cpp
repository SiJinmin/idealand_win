void idealand_threads_init()
{
  ...
  for (int i = 0; i < IdealandMaxThreads; i++) { p = IdealandThreads + i;  p->id = -1; p->status[0] = 0; p->pLog = NULL; p->log_path = NULL; }
}
...
{
  INT32 id = (INT32)GetCurrentThreadId(); IdealandThreadInfo* p = idealand_thread_by_id(id,0);
...
{
  INT32 id = (INT32)GetCurrentThreadId(); IdealandThreadInfo* p = idealand_thread_by_id(id); if (p == NULL || p->pLog == NULL || p->log_path==NULL) return;
  fclose(p->pLog);
  char* path_end = idealand_thread_log_path(id, 1); if (path_end == NULL) return;
  rename(p->log_path, path_end); free(p->log_path); free(path_end);
}
