...
int idealand_socket_address_get(char* hostName, int port, int addressFamily, int flags, struct addrinfo** ppResult)
{
  ...
  char portStr[64]; _itoa_s(IdealandListenPort, portStr, 63, 10);
  if (r = getaddrinfo(hostName, (PCSTR)portStr, &hints, ppResult)) 
  { idealand_socket_error(); idealand_log("getaddrinfo failed: %d\n", r); return -1;  }
