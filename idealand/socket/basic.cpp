...
int idealand_socket_set_timeout(SOCKET* p, int optname, INT32 timeout_milliseconds)
{
  ...
  if ((r = idealand_check_int(timeout_milliseconds, (char*)"timeout_milliseconds", (char*)__func__, 60000, 2000)) < 0) return r;
...
int idealand_socket_send(SOCKET* p, void* buf, int len)
{
  ...
  if ((r = idealand_check_int(len, (char*)"len", (char*)__func__, IdealandBufferSize, 0)) < 0) return r;
