...
int idealand_check_string(char* str, int allowNull, char* name, char* functionName, INT64 maxLen, INT64 minLen)
{
  if (str == NULL)
  {
    if (!allowNull) { idealand_log((char*)"%s cannot be NULL in %s", name, functionName); return -1; }
    else return 0;
  }
  else
  {
...
int idealand_check_filename(char* filename, char* name, char* functionName)
{
  ...
  if (r == 0) { idealand_log((char*)"%s cannot be empty in %s", name, functionName); r = -1;  } if(r<0) goto free1;
  if (r == 1 && *ppChars[0] == '.') { idealand_log("%s cannot be . in %s", name, functionName); r = -1; goto free1; }
  if (r >= 2 && *ppChars[0] == '.' && *ppChars[1] == '.') { idealand_log("%s cannot starts with .. in %s", name, functionName); r = -1; goto free1; }

  for (int i = 0; i < r; i++)
  {
    char c = *ppChars[i]; for (int j = 0; j < IdealandNameExcludeCharsCount; j++)
    {
      if (c == IdealandNameExcludeChars[j]) { idealand_log("%s cannot contain %c in %s", name, c, functionName); r = -1; break; }
    }
  }  
