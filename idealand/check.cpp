...
int idealand_check_string(const char* str, int allowNull, const char* name, const char* functionName, INT64 maxLen, INT64 minLen)
{
  if (str == NULL)
  {
    if (!allowNull) { idealand_error("%s cannot be NULL in %s", name, functionName); return -1; }
    else return 0;
  }
  else
  {
    INT64 len = strlen(str);
    if (len > maxLen) { idealand_error("%s(len=%lld) cannot be longer than %d in %s", name, len, maxLen, functionName); return -1; }
    if (len < minLen) { idealand_error("%s(len=%lld) cannot be shorter than %d in %s", name, len, maxLen, functionName); return -1; }
  }
  return 0;
}
...
int idealand_check_filename(const char* filename, const char* name, const char* functionName)
{
  ...
  if (r == 0) { idealand_error("%s cannot be empty in %s", name, functionName); r = -1;  } if(r<0) goto free1;  
