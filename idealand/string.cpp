...
char* idealand_string(int max, int *pcount, const char* format, ...)
{
  ...
  if (idealand_check_pointer((void *)format, "format", __func__) < 0) return NULL;
...
char *idealand_string_normalize_path(char* path, int dirEnd)
{
  ...
  if (dirEnd)
  {
    ...
    return idealand_string(len + 2, NULL, "%s%c", path, IdealandPathSep);
  }
  else return path;
}
...
char* idealand_string_time_length(INT64 seconds, int full, int* pcount)
{
  ...
  INT64 parts[4]; const char* partNames[] = {"seconds","minutes","hours","days"};
...
INT64 idealand_string_utf8(const char* str, INT64 *pAsciiCount, INT64* pOtherCount, char ***pppChars, int maxChars)
{
  int r = 0;
  if ((r = idealand_check_pointer((void *)str, "str", __func__)) < 0) return r;
  if ((r= idealand_check_malloc_size(maxChars, "maxChars", __func__)) < 0) { return r; }
  if (maxChars > 0) 
  { 
