...
日志文件位于idealand.exe所在的目录的IdealandLogs文件夹中，每个线程有单独的log文件，文件名为【日期时间_线程ID.log】
文件名中的日期时间为线程结束的时间，文件内容中会在开头和结尾显示线程的开始和结束时间，每条log信息会带有时间信息，精确到毫秒。
线程ID为0的log文件记录的是所有线程的log信息交织在一起的列表，用于综合查阅所有log信息，因此每条log信息前面会带有线程ID。
Console打印出的信息与线程ID为0的log文件内容基本一致，会有少量信息仅在Console显示而不会保存到log文件中。
